...
public class TweetParser {
    ...
    static List<String> csvDataToTweets(BufferedReader br, int tweetColumn) {
        ...
        while (fli.hasNext()) {
            String ex = extractColumn(fli.next(), tweetColumn);
            if (ex != null) {
                ls.add(ex);
            }
        }
        return ls; // Complete this method.
    }
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        //String clean = removeURLs(tweet);
        //cleanWord(clean);
        List<List<String>> ls = new LinkedList<>();
        //List<String> l = new LinkedList<>();
        for (String t: tweetSplit(removeURLs(tweet))) {
            //System.out.println(cleanWord(t));
            List<String> cs = parseAndCleanSentence(t);
            if (!cs.isEmpty()) {
                ls.add(cs);
            }
        }
    ...
    public static List<List<String>> csvDataToTrainingData(
        ...
        //List<String> ls = new LinkedList<>();
        for (String t: csvDataToTweets(br, tweetColumn)) {
            l.addAll(parseAndCleanTweet(t));
        }
